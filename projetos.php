<section id="div-projetos">

	<div>
		<h2>Meus Projetos</h2>

		<div id="dt_carousel" class="carousel slide dt_carousel">
				<div class="carousel-inner dt_carousel_inner">
					<div class="dt_carousel_item item active">
						<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12 dt_carousel_inner_item_div">
							<img class="img-responsive" src="<?php echo $site_url?>/images/350x400.gif" alt="Banner 1">
						</div>

						<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12 dt_carousel_inner_item_div">
							<img class="img-responsive" src="<?php echo $site_url?>/images/350x400(1).gif" alt="Banner 2">
						</div>

						<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12 dt_carousel_inner_item_div">
							<img class="img-responsive" src="<?php echo $site_url?>/images/350x400(2).gif" alt="Banner 3">
						</div>

						<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12 dt_carousel_inner_item_div">
							<img class="img-responsive" src="<?php echo $site_url?>/images/350x400(3).gif" alt="Banner 3">
						</div>
					</div>

					<div class="dt_carousel_item item">
						<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12 dt_carousel_inner_item_div">
							<img class="img-responsive" src="<?php echo $site_url?>/images/350x400(1).gif" alt="Banner 2">
						</div>

						<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12 dt_carousel_inner_item_div">
							<img class="img-responsive" src="<?php echo $site_url?>/images/350x400(2).gif" alt="Banner 3">
						</div>

						<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12 dt_carousel_inner_item_div">
							<img class="img-responsive" src="<?php echo $site_url?>/images/350x400(3).gif" alt="Banner 3">
						</div>

						<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12 dt_carousel_inner_item_div">
							<img class="img-responsive" src="<?php echo $site_url?>/images/350x400(4).gif" alt="Banner 3">
						</div>
					</div>

					<div class="dt_carousel_item item">
						<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12 dt_carousel_inner_item_div">
							<img class="img-responsive" src="<?php echo $site_url?>/images/350x400(2).gif" alt="Banner 3">
						</div>

						<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12 dt_carousel_inner_item_div">
							<img class="img-responsive" src="<?php echo $site_url?>/images/350x400(3).gif" alt="Banner 3">
						</div>

						<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12 dt_carousel_inner_item_div">
							<img class="img-responsive" src="<?php echo $site_url?>/images/350x400(4).gif" alt="Banner 3">
						</div>

						<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12 dt_carousel_inner_item_div">
							<img class="img-responsive" src="<?php echo $site_url?>/images/350x400.gif" alt="Banner 1">
						</div>
					</div>

					<div class="dt_carousel_item item">
						<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12 dt_carousel_inner_item_div">
							<img class="img-responsive" src="<?php echo $site_url?>/images/350x400(3).gif" alt="Banner 3">
						</div>

						<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12 dt_carousel_inner_item_div">
							<img class="img-responsive" src="<?php echo $site_url?>/images/350x400(4).gif" alt="Banner 3">
						</div>

						<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12 dt_carousel_inner_item_div">
							<img class="img-responsive" src="<?php echo $site_url?>/images/350x400.gif" alt="Banner 1">
						</div>

						<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12 dt_carousel_inner_item_div">
							<img class="img-responsive" src="<?php echo $site_url?>/images/350x400(1).gif" alt="Banner 2">
						</div>
					</div>

					<div class="dt_carousel_item item">
						<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12 dt_carousel_inner_item_div">
							<img class="img-responsive" src="<?php echo $site_url?>/images/350x400(4).gif" alt="Banner 3">
						</div>

						<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12 dt_carousel_inner_item_div">
							<img class="img-responsive" src="<?php echo $site_url?>/images/350x400.gif" alt="Banner 1">
						</div>

						<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12 dt_carousel_inner_item_div">
							<img class="img-responsive" src="<?php echo $site_url?>/images/350x400(1).gif" alt="Banner 2">
						</div>

						<div class="col-lg-3 col-md-4 col-sm-6 col-xs-12 dt_carousel_inner_item_div">
							<img class="img-responsive" src="<?php echo $site_url?>/images/350x400(3).gif" alt="Banner 3">
						</div>
					</div>

					<a href="#dt_carousel" class="left carousel-control  dt_carousel_control dt_carousel_controle_left" data-slide="prev"><img alt="Carousel Dreamtech Left Arrow" src="<?php echo $site_url?>/images/arrow_left.png"></a>

					<a href="#dt_carousel" class="right carousel-control  dt_carousel_control dt_carousel_controle_right" data-slide="next"><img alt="Carousel Dreamtech Right Arrow" src="<?php echo $site_url?>/images/arrow_right.png"></a>

				</div>
		</div>
	</div>

</section>